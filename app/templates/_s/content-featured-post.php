...
<article id="post-<?php the_ID(); ?>" <?php post_class($classes = array('article', 'article--featured-post')); ?>>
    ...
    <header class="header  article__header  article--featured-post__header">
        ...
        <div class="header__meta  article__header__meta  article--featured-post__header__meta">
            <span class="cat-links"><?php echo get_the_category_list( _x( ', ', 'Used between list items, there is a space after the comma.', '_s' ) ); ?></span>
        </div><!-- .entry-meta -->
        <?php endif; ?>

        <?php the_title( '<h1 class="header__title  article__header__title  article--featured-post__header__title"><a href="' . esc_url( get_permalink() ) . '" rel="bookmark">','</a></h1>' ); ?>
    </header><!-- .entry-header -->
</article><!-- #post-## -->
